...
const EventReportForm: React.FC<EventReportFormProps> = ({ onSubmit, onCancel, eventId, loading, type, initialValues }) => {
  ...
  useEffect(() => {
    if (initialValues) {
      setForm({
        ...ReportFormState,
        ...initialValues,
        // convert array to string for achievements and resultImages if needed
        achievements: Array.isArray(initialValues.achievements) ? initialValues.achievements.join("\n") : initialValues.achievements || "",
        resultImages: Array.isArray(initialValues.resultImages) ? initialValues.resultImages.join(",") : initialValues.resultImages || "",
      });
    } else {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSubmit({
      ...form,
      eventId,
      resultImages: form.resultImages.split(",").map((s) => s.trim()),
        ...
        <DialogContent dividers>
          <Box sx={{ mt: 1, mb: 1 }}>
            <Grid container spacing={2}>
              ...
              <Grid item xs={12}>
                <ImageUploader
                  currentImage={form.resultImages}
